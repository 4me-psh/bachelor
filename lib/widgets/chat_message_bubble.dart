import 'package:flutter/material.dart';
import '../models/chat_message.dart';
import '../models/clothing_item.dart';

class ChatMessageBubble extends StatelessWidget {
  final ChatMessage message;
  final VoidCallback? onRepeat;
  final VoidCallback? onOpenDetails;

  const ChatMessageBubble({
    super.key,
    required this.message,
    this.onRepeat,
    this.onOpenDetails,
  });

  @override
  Widget build(BuildContext context) {
    if (message.isUser) {
      return Align(
        alignment: Alignment.centerRight,
        child: Container(
          margin: const EdgeInsets.symmetric(vertical: 6, horizontal: 12),
          padding: const EdgeInsets.all(14),
          constraints: BoxConstraints(maxWidth: MediaQuery.of(context).size.width * 0.75),
          decoration: BoxDecoration(
            color: const Color.fromARGB(82, 141, 69, 35),
            borderRadius: BorderRadius.circular(16),
            boxShadow: [
              BoxShadow(
                color: Color.fromARGB(60, 93, 62, 9),
                blurRadius: 6,
                offset: Offset(0, 3),
              ),
            ],
          ),
          child: Text(
            message.userMessage!,
            style: const TextStyle(color: Colors.white, fontSize: 16),
          ),
        ),
      );
    }

    if (message.isRecommendation) {
      final recommendation = message.recommendation!;
      final items = recommendation.recommendedClothes ?? [];
      final images = recommendation.generatedImages ?? [];

      return Align(
        alignment: Alignment.centerLeft,
        child: Container(
          margin: const EdgeInsets.symmetric(vertical: 8, horizontal: 12),
          padding: const EdgeInsets.all(14),
          decoration: BoxDecoration(
            color: const Color.fromARGB(26, 212, 94, 40),
            borderRadius: BorderRadius.circular(16),
            boxShadow: [
              BoxShadow(
                color: Color.fromARGB(62, 160, 107, 16),
                blurRadius: 6,
                offset: Offset(0, 3),
              ),
            ],
          ),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              if (images.isNotEmpty) ...[
                SizedBox(
                  height: 120,
                  child: ListView.separated(
                    scrollDirection: Axis.horizontal,
                    itemCount: images.length,
                    itemBuilder: (context, index) {
                      return ClipRRect(
                        borderRadius: BorderRadius.circular(10),
                        child: Image.network(
                          images[index],
                          width: 100,
                          height: 100,
                          fit: BoxFit.cover,
                        ),
                      );
                    },
                    separatorBuilder: (_, __) => const SizedBox(width: 10),
                  ),
                ),
                const SizedBox(height: 12),
              ],

              if (items.isNotEmpty)
                Wrap(
                  spacing: 12,
                  runSpacing: 12,
                  children: items.map(_clothingPreview).toList(),
                ),

              const SizedBox(height: 10),

              Row(
                children: [
                  TextButton.icon(
                    onPressed: onRepeat,
                    icon: const Icon(Icons.refresh, size: 20),
                    label: const Text('Повторити'),
                    style: TextButton.styleFrom(foregroundColor: const Color.fromARGB(118, 189, 122, 22)),
                  ),
                  const Spacer(),
                  TextButton(
                    onPressed: onOpenDetails,
                    child: const Text('Детальніше'),
                  ),
                ],
              ),
            ],
          ),
        ),
      );
    }

    return const SizedBox.shrink();
  }

  Widget _clothingPreview(ClothingItem item) {
    return SizedBox(
      width: 110,
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.center,
        children: [
          ClipRRect(
            borderRadius: BorderRadius.circular(10),
            child: Image.network(
              item.getImage(useRemovedBg: item.useRemovedBg),
              width: 100,
              height: 100,
              fit: BoxFit.cover,
            ),
          ),
          const SizedBox(height: 6),
          Text(
            item.name,
            style: const TextStyle(fontWeight: FontWeight.bold),
            textAlign: TextAlign.center,
            maxLines: 1,
            overflow: TextOverflow.ellipsis,
          ),
          Text(
            item.pieceCategory.nameUa,
            style: const TextStyle(fontSize: 12, color: Color.fromARGB(255, 41, 35, 35)),
            textAlign: TextAlign.center,
          ),
        ],
      ),
    );
  }
}
